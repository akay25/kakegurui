...
export default {
  ...
  methods: {
    ...
    leaveThisPage(data) {
      ...
      // For some reason, when triggering router.push is not calling socket.connect on the next page
      // Hence, forcing a page reload
      window.location.href = `/room/${data.room.name}`;
